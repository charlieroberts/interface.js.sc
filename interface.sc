Interface : Object {
  classvar <>remoteIP, <>remotePort, <oscout, widgetNumber, widgets, <>client, <>prefix;
  
  *init {arg ip = "127.0.0.1", port = 10000;
    remoteIP = ip;
    remotePort =  port;
    "Interface INIT".postln;
    ...
    oscout.sendMsg("/createLivecodeServer", 57120)
  }
  ...
  *make {arg args;
    ...
    widget = InterfaceWidget.new(this, args);
  ...
  *removeWidget{ arg name;
    ...
    oscout.sendMsg( "/interface/removeWidget", name);
  }
  
  *sendOSC { arg address, value;
    if( (prefix != -1 ), {
      oscout.sendMsg(prefix ++ address, value);
    },{
...
InterfaceWidget : Object {
  ...
  *new { arg master, attributes;
  ...
  init {
    ...
    master.sendOSC( "/interface/addWidget", json);
